		...
		else
		{
			...
			for(y=y0,vv=y*vi+v;y<y1;y++,vv+=vi)
			{
				...
				"begdtnhalf:\n\t"
					...
					"jz	preenddtnhalf\n\t"
					"shr	$16,%eax\n\t"
					"punpckldq	(%ebx,%eax,4),%mm0\n\t"
					"movntq	%mm0,-4(%edi,%edx,4)\n\t"
					"add	$1,%edx\n\t"
					"jnz	begdtnhalf\n\t"
					"jmp	enddtnhalf\n"
				"preenddtnhalf:\n\t"
...
//---------------------------- CPU Tests, etc ---------------------------
	...
	/*#if CPUEXTN == 1 //Actual runtime selecting (macro stuff & compile-time alternatives at top)
		if (cputype&(1<<25)) //Runtime selection: SSE instructions
		{
			using namespace cpu_sse_specific_code;
		} else if ((cputype&((1<<30)|(1<<31))) == ((1<<30)|(1<<31))) //Runtime selection: Runtime selection
		{
			using namespace cpu_3dn_specific_code;
		}
	#endif*/
